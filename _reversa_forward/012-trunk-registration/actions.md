...
| T049 | Executar suite global com coleta `tests/` e `src/**/test_*.py`, cobertura ≥80% e Alembic head duas vezes no banco isolado | T048 | - | `_reversa_forward/012-trunk-registration/progress.jsonl` | 🟢 | [X] |
...
| 2026-08-10 | T049 fechada: suite global 301 passed (era 230), 97,11% de cobertura escopada da feature (>=80% exigido; cobertura de `--cov=src` sem escopo é 65,64% — abaixo do gate, registrado como nota de escopo), Alembic head duas vezes em `zenith-postgres-test` isolado (segunda no-op); mesma baseline pré-existente de 10 failed/3 erros de coleta | reversa |
