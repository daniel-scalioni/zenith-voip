...
|---------|----------|---------------|
| Auditor (equipe externa) | Avaliar qualidade de atendimento em paralelo ao sistema de interfonia | Acessa o share conforme a política administrativa do storage, busca áudios por data e ramal (origem/destino) e escuta a conversa para validar conformidade |
...
| RF-07 | Disponibilizar o áudio no share SMB; a política de contas humanas é administrada no storage e não integra o contrato do Zenith | Should | A conta técnica configurada publica e relê o arquivo para checksum; o responsável confirma o acesso e a reprodução do áudio remoto | 🟢 |
...
| RF-10 | Antes do E2E, configurar manualmente o `.env` privado do deploy com endpoint SMB e uma conta técnica com WRITE e READ para publicação e checksum | Must | `.env` contém host, porta, modo Direct TCP, nomes NetBIOS, share, path, usuário e senha técnica; `.env.example` contém somente placeholders; nenhum segredo aparece em Git, specs ou logs | 🟢 |
