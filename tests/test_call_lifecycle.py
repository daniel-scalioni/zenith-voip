...
async def test_channel_answer_creates_call_row():
    ...
    # Act
    with patch("src.telephony.esl_client.create_call_record", new=AsyncMock()) as create, patch(
        "src.telephony.esl_client.mark_call_in_progress", new=AsyncMock(return_value=False)
    ), patch.object(client, "_start_audio_capture", new=AsyncMock()):
...
async def test_channel_hangup_finalizes_call_and_recording_once():
    ...
    # Assert
    finalize.assert_awaited_once_with("tenant-1", "call-xyz", None)
